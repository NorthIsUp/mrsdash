...
def accepts(*types, **kw):
    ...
    """
    debug = kw['debug'] if 'debug' in kw else 1
    self = kw['self'] if 'self' in kw else False
    try:
        def decorator(f):
            @wraps(f)
            def newf(*args, **kwargs):
                ...
                if debug == 0:
                    return f(*args)
                assert len(args) - self == len(types)
                argtypes = tuple(map(type, args[self:]))
                if argtypes != types:
                    msg = info(f.__name__, types, argtypes, 0)
                    if debug == 1:
                        logger.debug("TypeWarning: %s", msg)
                    elif debug == 2:
                        raise TypeError(msg)
